...
function loadWallet() {
  ...
  let secretKey;
  try {secretKey = Uint8Array.from(JSON.parse(secret));} catch (err) {// preserve original parse errorthrow new Error(‘ANCHR_WALLET_SECRET_KEY must be a JSON array of numbers (e.g. the contents of a Solana CLI keypair file).’,{ cause: err });}
...
export async function writeIpfsRecord(domain, cid, { rpcUrl = DEFAULT_RPC } = {}) {
  ...
  try {
    ...
    console.log(`(Record likely already exists for ${domain} — falling back to update: ${createErr.message})`);
    try {
      ...
      // Attach the caught updateErr as the cause so the original stack/message
      // are preserved and the ESLint rule is satisfied.
      throw new Error(
        `Both create and update attempts failed for ${domain}.\n` +
        `Create error: ${createErr?.message ?? String(createErr)}\n` +
        `Update error: ${updateErr?.message ?? String(updateErr)}`,
        { cause: updateErr }
      );
    }
  }
